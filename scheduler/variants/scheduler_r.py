...
class Scheduler_r(Scheduler):
    ...
    Args:
        ...
        allocation (dict): Dictionary representing worker allocations as {worker_id: "start_time-end_time"}.
    ...
    def __init__(self, accuracy, allocation, variant):
        ...
        self.allocation = allocation        # {1:"7:00-10:00", ...} z tego mozna wyciagnac kiedyy poczatek i koniec pracy i ilosc pracownnikow
    ...
    def _get_least_used_workers(self):
        ...
        if least_used_workers:
            return random.choice(least_used_workers)
        else:
            return None
    ...
    def make_schedule(self):
        ...
        time_frames_number = self._number_of_time_frames_per_day(self.accuracy)
        ...
        for day in days:
            for position in positions:
                for time_frame in range(1, time_frames_number + 1):
                    needed_workers = self.allocation[position]
                    assigned_workers = []

                    while len(assigned_workers) < needed_workers:
                        # Try previous worker
                        previous_worker = self._get_previous_time_frame_worker(day, time_frame)
                        if previous_worker is not None and previous_worker.is_available(day, time_frame):
                            assigned_workers.append(previous_worker)
                            continue

                        # Try to prioritize the least used worker
                        least_used_worker = self._get_least_used_workers()
                        available_workers = self.worker_manager.get_available_workers(day, time_frame)
                        for worker in least_used_worker:
                            if worker.position == position and worker in available_workers:
                                assigned_workers.append(worker)
                                break

                        # Take with normal availability
                        for worker in available_workers:
                            if worker.position == position:
                                assigned_workers.append(worker)
                                break

                        if len(assigned_workers) < needed_workers:
                            available_workers_if_needed = self.worker_manager.get_available_workers_if_needed(day,
                                                                                                              time_frame)
                            # Loop with worse availability with prio the least used worker
                            for worker in least_used_worker:
                                if worker.position == position and worker in available_workers_if_needed:
                                    assigned_workers.append(worker)
                                    break
